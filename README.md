...
- [ ] **作品スクショ/動画の差し替え**(残っているもの)
  ...
  - ⚠️ **メルカリ出品アシスタント**: `images/mercari-demo.jpg` として実スクショ(AI生成結果画面)を用意済みだが**未使用**。
    元画像は開発時のdocsフォルダに残っていたもので、実アカウントの定型文(`#古着屋36`)が写っていたため黒塗り処理済み。
    `works/mercari.html`(案件詳細ページ)が無いため、まだどこにも埋め込まれていない — ページ作成時にヒーロー画像として使うこと
  - [ ] **CSチャットボット(p4)**: Vercelに未デプロイのため実スクショが撮れていない。デプロイしてから撮影
  - スマホ幅で文字が読めるかを確認
